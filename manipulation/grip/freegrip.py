...
class Freegrip(fgcp.FreegripContactpairs):
    ...
    def removeHndcc(self, base, discretesize=1):
        """
        ...
        while self.counter < self.facetpairs.shape[0]:
            ...
            for j, contactpair in enumerate(self.gripcontactpairs_precc[self.counter]):
                for angleid in range(discretesize):
                    ...
                    # tmphand = self.hand
                    tmphand = rtq85nm.Rtq85NM(hndcolor=[1, 0, 0, .1])
                    ...
                    # gndcollection = tmphand.handnp.findAllMatches("**/+GeomNode")
                    # print "len of gnd collection ", len(gndcollection)
                    # geombullmesh = GeomNode('test')
                    # for gnd in gndcollection:
                    #     print gnd.node().getGeom(0)
                        # geombullmesh.addGeom()
                    # test = NodePath('test')
                    # test.attachNewNode(geombullmesh)
                    ...
                    if not result.getNumContacts():
                        ...
                        # tmphand.setColor([.5, .5, .5, 1])
                        # tmphand.handnp.reparentTo(base.render)
                        # self.rtq85plotlist.append(tmprtq85)
                        # isplotted = 1

                    # reset initial hand pose
                    tmphand.setMat(initmat)
                if len(self.gripcontacts) > 0:
                    break
            self.counter+=1
            if len(self.gripcontacts) > 0:
                break
