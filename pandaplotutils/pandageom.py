...
def plotDumbbell(nodepath, spos = None, epos = None, length = None, thickness = 0.01, rgba=None, plotname="dumbbell", headscale = 0.01):
    """
...
def generateFFPlacement(objtrimesh, direction, mass_center, doverh=0.1):
    ...
    if len(faceSide) <= 3:
        ...
        for p in range(len(faceSide)): # remind: the first point and the last point is the same point
            ...
            elif p == len(faceSide) - 1:
                if not inSameLine(faceSide[p-1], faceSide[p], faceSide[0]):
                    verts2d.append(faceSide[p])
