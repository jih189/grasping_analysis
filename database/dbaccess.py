...
class GraspDB(object):
    ...
    def __init__(self):
        self.dbconnection = mdb.connect("localhost", "cogrob", "cogrob", "freegrip")
    ...
    def loadFreeTabletopPlacementIncludeFF(self, objname):
        ...
        # access to db
        sql = "SELECT freetabletopplacement.rotmat, freetabletopplacement.placement, \
                freetabletopplacement.idfreetabletopplacement FROM freetabletopplacement,object WHERE \
                freetabletopplacement.idobject = object.idobject \
                AND object.name LIKE '%s'" % objname
        data = self.execute(sql)
        if len(data) != 0:
            for i in range(len(data)):
                tpsmat4s.append(dc.strToMat4(data[i][0]))
                placementtype.append(data[i][1])
                placementid.append(data[i][2])
    ...
    def loadIKRet(self):
        # by default there is no ikret table, so it will be always 50,50,50
        # rethandx: the distance of retract along handx direction, default 50mm
        # retworldz: the distance of retract along worldz direction, default 50mm
        # retworlda: the distance of retract along assembly/approaching direction in the world, default 50mm
